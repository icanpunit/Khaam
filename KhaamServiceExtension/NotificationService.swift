...
//  KhaamServiceExtension
//
//  Created by baps on 07/07/20.
...
class NotificationService: UNNotificationServiceExtension {
    ...
    override func didReceive(_ request: UNNotificationRequest, withContentHandler contentHandler: @escaping (UNNotificationContent) -> Void) {
        ...
        let DataDic:NSDictionary = (request.content.userInfo as NSDictionary?)!
        
        
        guard let attachmentUrlString = (DataDic.value(forKey: "fcm_options") as! NSDictionary).value(forKey: "image") as? String else {
            return
        }
