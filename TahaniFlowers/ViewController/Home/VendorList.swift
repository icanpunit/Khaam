...
class VendorList: UIViewController,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,UISearchBarDelegate,HttpWrapperDelegate,UITabBarDelegate,UITabBarControllerDelegate {
...
//            isSubCategory = false
        ...
        self.lblNoDataFound.text = "lbl_Category_NoData_Title".setlocalized()
        self.lblNODataDetail.text = "lbl_Category_NoData_Detail".setlocalized()
...
//        navigationController?.setViewControllers(viewControllers!, animated: true)
    ...
    @objc func gotoProductListing(sender:UITapGestureRecognizer) {
        ...
        if let dictProduct = self.arrCategoryList.object(at: index!) as? NSDictionary {
            ...
            if let nCategoryId = dictProduct.object(forKey: "nSupplierId") as? NSNumber {
                               let nextViewController = objHomeSB.instantiateViewController(withIdentifier: "VendorProductlist") as! VendorProductlist
                               nextViewController.nvendorid = "\(nCategoryId)"
                               if let cCategoryName = dictProduct.object(forKey: "cStoreName") as? String {
    ...
    func getCategoryList() {
        if net.isReachable == false {
            ...
            param = String(format: "%@?nUserId=%@&nLanguageId=%@&cToken=%@", kSelectAllSupplier,"\(nUserId)","\(nLanguageId)","\(cToken)")
    ...
    func addNavigationBar(){
        ...
        lblTitle.text = "lbl_vendorlist_Title".setlocalized()
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let dicProduct = arrCategoryList.object(at: indexPath.row) as? NSDictionary {
           ...
           if UserDefaults.standard.object(forKey: kLanguageId) != nil {
            ...
            if nLanguageId == "1"
            {
                if let nameProduct = dicProduct.object(forKey: "cStoreName") as? String {
                               cell.lblProductCategoryName.text = nameProduct
                           }
            }
            else
            {
                if let nameProduct = dicProduct.object(forKey: "cStoreNameAribic") as? String {
                               cell.lblProductCategoryName.text = nameProduct
                           }
            }
            ...
            if let urlImage = dicProduct.object(forKey: "cStoreImage") as? String {
